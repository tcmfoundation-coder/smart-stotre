...
jest.mock('@/models', () => ({
  StockAdjustment: { find: jest.fn(), findById: jest.fn(), create: jest.fn() },
...
describe('POST /api/stock-adjustments/[id]/approve', () => {
  ...
  it('applies the stock change against the live product quantity, not the stale estimate', async () => {
    ...
    (StockAdjustment.findById as jest.Mock).mockResolvedValue(adjustment);
  ...
  it('blocks approval that would drive stock negative', async () => {
    ...
    (StockAdjustment.findById as jest.Mock).mockResolvedValue(adjustment);
    (Product.findById as jest.Mock).mockResolvedValue({ stockQuantity: 5, name: 'Widget', save: jest.fn() });
